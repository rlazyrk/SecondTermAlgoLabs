...
public List<Garden> findAllWithOrchard() {
    return gardens.stream().filter(garden -> garden.hasOrchard()!= false).toList();
}
...
public static void main (String... args){
    ...
    for(Garden garden : gardenManager.gardens) System.out.println(garden.toString()+ "\n" );
    var allWithOrchard = gardenManager.findAllWithOrchard();
    for(Garden garden: allWithOrchard) System.out.println(garden.toString());
    ...
    for(Garden garden: moreThan100Flower) System.out.println(garden.toString());
    }
}
